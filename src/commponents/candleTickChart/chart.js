...
import { BarSeries,CandlestickSeries,LineSeries,MACDSeries,RSISeries,AreaSeries,StraightLine,ScatterSeries,CircleMarker, } from "react-stockcharts/lib/series";
...
class CandleStickChart extends React.Component {
	...
	render() {
		...
		let calculateData = this.calculateData(initialData);
					...
					xExtents={xExtents}>
					{
						...
						{currentChart.name === 'Line' && (
						  <Chart id={1} height={chartHeight - 60} yExtents={d => [d.high, d.low]}>
							...
							<OHLCTooltip forChart={1} origin={[-40, 0]} {...mouseEdgeAppearance}/>
						  </Chart>
						)}
						{currentChart.name === 'Heikin Ashi' && (
						  ...
						  >
							...
							<OHLCTooltip origin={[-40, 0]} {...mouseEdgeAppearance}/>
							<MovingAverageTooltip
							  origin={[-28, 15]}
							  options={[
								{
